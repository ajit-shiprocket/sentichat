...
func (store *MessageStore) GetMessages(chatJID string, limit int) ([]Message, error) {
	rows, err := store.db.Query("SELECT id, chat_jid, sender, content, timestamp, is_from_me FROM messages WHERE chat_jid = ? GROUP BY id ORDER BY timestamp DESC LIMIT ?", chatJID, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
